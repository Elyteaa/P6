...
cout << "Rotvec1 = " << rotvec << endl;
rotvec = rotvec.normalized();
cout << "rotvec2 = " << rotvec << endl;
/*
...
float pitch = asin(vectorFromEndEffector(1)); //pitch
float yaw = atan2(vectorFromEndEffector(0),vectorFromEndEffector(2)); //yaw

//Converting orientation to quaternions
q = AngleAxisf(pose[3], Vector3f::UnitX())
...
//q.FromTwoVectors(vectorFromEndEffector, planevec);
...
if (abs(finIdx-point1)<10 || abs(finIdx-point2)<10)
...
output.pose.position.x = pose1[0];
output.pose.position.y = pose1[1];
output.pose.position.z = pose1[2];
output.pose.orientation.w = pose1[3];
output.pose.orientation.x = pose1[4];
output.pose.orientation.y = pose1[5];
output.pose.orientation.z = pose1[6];
