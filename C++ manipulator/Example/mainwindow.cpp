...
void MainWindow::on_btnTestButton_clicked(){
    ...
    //Move the robot home
    tJoints home_joints;
    double home_values [6] = {0, -90, -90, 0, 90, 0};
    const double * phome_values = home_values;
    home_joints.SetValues(phome_values, 6);
    //Item solver;
    Mat home_position = ROBOT->SolveFK(home_joints, nullptr, nullptr);
    //ROBOT->MoveL(home_joints);
    ROBOT->MoveL(home_position);
    qDebug() << "Moved to the home position";
